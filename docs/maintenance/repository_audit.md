...
## Packaging
- **name:** `nirs4all-core (PyPI) / nirs4all (crate, npm) / nirs4all (R, MATLAB)` — **version:** `0.2.0`
- **issues:**
- Repo/PyPI rename nirs4all-lite -> nirs4all-core is a PENDING external admin action: pyproject [project.urls] and Cargo repository still point to github.com/GBeurier/nirs4all-lite; RTD slug/OG URLs in docs/conf.py are still nirs4all-lite.readthedocs.io
...
- Aggregate depends on upstreams that must already be published at the pinned versions: crate depends on dag-ml 0.2.0 + dag-ml-data 0.2.0 on crates.io; pyproject extras require nirs4all-formats>=0.1.0 / nirs4all-io>=0.1.0 / nirs4all-methods>=1.0.0 / dag-ml>=0.2.0 on PyPI — release/install fails if any is absent
...
- **status:** Looks buildable (self-consistent conf.py, pinned doc deps, static brand assets present). Weakness: no API autodoc (pure narrative), and no CI docs-build job so RTD is the only place breakage surfaces; OG/site URLs still hardcode the pre-rename nirs4all-lite.readthedocs.io slug.
...
- Bump classifiers from 'Development Status :: 3 - Alpha' to Beta once the rename/publishers land, and reconcile the nirs4all-lite URLs/RTD slug in pyproject.toml, Cargo repository, and docs/conf.py as part of the CORE_RENAME cutover.

## Deepest hardening roadmap (fullest realistic hardening)
- Complete the nirs4all-lite -> nirs4all-core rename cutover (docs/CORE_RENAME.md Phase R2): repo rename, PyPI Trusted Publisher creation for nirs4all-core, claim crates.io + npm 'nirs4all' names, then update all Homepage/Repository/Issues URLs and the RTD/OG slug in docs/conf.py.
...
- Pushing to main does NOT itself publish (all publish jobs gate on refs/tags/v and non-pre-release), so main is safe from accidental release — but it DOES trigger the full CI including the expensive strict-parity job that clones external GBeurier/nirs4all-methods (pinned SHA 6f6a3fa in ci.yml, and the compat/upstreams.toml ref in release-npm.yml) and builds emscripten/R/octave; upstream unavailability can turn a main push red.
